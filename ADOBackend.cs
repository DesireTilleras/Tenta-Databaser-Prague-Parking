...
namespace HemtentaDesireDatabaserPragueParking
{
    class ADOBackend
    {
        ...
        /// <returns></returns>
        public int BackEndCheckIn(string regNr, int vehicleType)
        {
            int parkingSpot = 0;
       ...
       /// <returns></returns>
        ...
        /// </summary>
        /// <returns></returns>
        public List<Vehicle> ListAllSpots()
        {
       ...
       /// <returns></returns>
        public int AverageMoneyPerDay(DateTime fromDate, DateTime toDate)
        {
            int money;
            string sqlQuery = "SELECT dbo.fk_AveragePerDay(@fromDate, @toDate)";
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                SqlCommand command = new SqlCommand(sqlQuery, connection);
                try
       ...
       /// <returns></returns>
        ...
        /// <returns></returns>
        public int MoneySpecDay(DateTime date)
        {
            int money;
            string sqlQuery = "SELECT dbo.fk_SpecDate(@date)";
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                SqlCommand command = new SqlCommand(sqlQuery, connection);
                try
        ...
        /// </summary>
        /// <returns></returns>
        public List<string> Vehicle48h() // OBS!! DESSI Glöm ej ändra till view "Vehicles48h" innan inlämning!!!!!
        {
            List<string> listOfRegNr48h = new List<string>();

            string sqlQuery = "SELECT CAST(SpotID AS NVARCHAR(20)) AS Spot, RegNumber, CAST(CheckInTime AS NVARCHAR(30)) AS CheckInTime" +
                                " FROM ParkedVehicles" +
                                " WHERE DATEDIFF(HOUR, CheckInTime, GETDATE())> 20 ";
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    ...
                    while (reader.Read())
                    {
                        ...
                        listOfRegNr48h.Add($"Spot: {spot} | Regnr :{regNr} | Arrival Time : {arrivalTime}\n");
                    }
                }
                catch (Exception)
                {
        ...
        /// </summary>
        /// <returns></returns>
        public List<Vehicle> ListAllHistory()
        {
            List<Vehicle> returnList = new List<Vehicle>();
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                try
                {
                    ...
                    {                        
                        ...
                        Vehicle toBeAdded = new Vehicle(regNumber, checkInTime, checkOutTime, totalPrice, null, null);
                        returnList.Add(toBeAdded);
                    }
                }
                catch (Exception)
